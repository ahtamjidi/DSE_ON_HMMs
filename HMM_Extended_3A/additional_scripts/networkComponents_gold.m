...
function [size_group,nComponents,members] = networkComponents_gold(A)
% Number of nodes
N = size(A,1);
% Remove diagonals
A(1:N+1:end) = 0;
    ...
    if ~isDiscovered(n)
        ...
        while (ptr <= length(members{end}))
            ...
            members{end}(end+1:end+length(newNbrs)) = newNbrs;
                    nComponents(newNbrs) = numel(members);
...
nComponents_ = length(members);
for n=1:nComponents_
    % compute sizes of components
    group_n = members{n};
    for j=1:length(group_n)
