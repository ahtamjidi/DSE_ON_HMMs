function [HYB2CEN,ICF2CEN,FHS2CEN] = EstsDist2CEN(Sim,Network)
    for k = 1:Sim.EndTime 
        for i = 1:Network.NumNodes    
            ...
            temp = Network.CEN_Est.Post(:,k)./Network.Node(i).FHS_Est.Post(:,k);
            FHS2CEN.ProjMetric(i,k) = log(max(temp)/min(temp));
            FHS2CEN.L1(i,k) = sum(abs(Network.CEN_Est.Post(:,k)-Network.Node(i).FHS_Est.Post(:,k)));            
        end
    end
    ...
    FHS2CEN.meanProjMetric = mean(FHS2CEN.ProjMetric);
    FHS2CEN.meanL1 = mean(FHS2CEN.L1);    
end