function [Network] = AMD_ENT(Global,Network,k)
...
% Observations (#4)
    if Network.isConnected(k) ~= 1
        ...
        %% Optimization
        Cost = @(omega)EntropyCostFuncAMD(omega,Network,k);
%         Cost = @(omega)KLDCostFuncAMD(omega,Network,k);
...
%         omega = .5*ones(1,Global.NumStates);%[.5 ;.5];
        %%
