function Network = IterativeConservativeFusion(Network,Global,Method,k)
switch Method
    case 'GMD'
        Network = NGMD_ICF(Network,Global,k);
    case 'GCF'
        Network = NGCF_ICF(Network,Global,k);
end

end
function Network = NGMD_ICF(Network,Global,k)
...
function Network = NGCF_ICF(Network,Global,k)
% initialzie consensus variables
for iConsensus=1:Global.ConsensusIter
    for iNode=1:Global.Graph.Params.NumNodes
        if iConsensus ==1
